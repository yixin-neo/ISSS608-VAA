...
- tooltip function: 
 - it takes in two arguments (y and ymax) from stat_summary via after_stat()
 - scales:: number is to convert number to tet, with formatting.
 - sem output: if ymax = y + se , then in tooltip, sem = ymax-y, isnt sem = se?
 - it outputs "mean maths scores: 57.44 +/- 2.03"

- the first stat summary function has two aes mappings to visual: (1)MATHS as Y (2)tooltip output
  - it first applies mean_se to get y, ymin, ymax for each X value
...
- the second stat summary uses y,ymin, ymax in mean_se to plot the error bar

**DOUCUMENTATION**
fun.data='mean_se' is default. It returns a data frame with three columns:
mean_se(x, mult = 1)
