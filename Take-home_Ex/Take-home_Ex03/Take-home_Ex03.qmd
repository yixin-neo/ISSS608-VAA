...
1.  Use visual analytics to identify anomalies in the business groups present in the knowledge graph provided by FishEye Organisation.
2.  Based on the visualizations on Network Risk, Financial Risk and Country Risk, suggest some anomalous companies that could be involved in illegal fishing.
...
In this section, we will detect anomalies in three aspects:

1.  Identifying subgraphs (a group of business relationships) with **high network diameter**. Subgraphs with higher network diameter would contain more companies and through such graphs, we can identify beneficial owners who owns more than 1 company.
2.  Identifying subgraphs with **high average revenue**.
3.  Identifying subgraphs associated with **a lot of countries**.
...
At a glance, there are two subgraphs (bottom) with 5 high betweenness nodes connected to one another, forming a larger than normal network. We are interested in subgraphs like these with longer network diameter because they represent more complex business relationship. For instance, in such complex subgraphs, entities involved are 'company', 'business owners' and 'company contacts' and there are also more than 1 company in the network. From literature review, having entities associated to two or more companies (conflict of interests could occur) could suggests transshipment activities. This could allows us to investigate the entities in the complex subgraphs to check for any IUU crime.

## 4.1 High NETWORK RISK

In this section, instead of having to 'eye-ball' each of the seafood network subgraphs above, to identify those with longer network diameter, we can extract edge data of each subgraph (using `membership` column) and compile the network diameter of each subgraph into a dataframe.

### 4.1.1 Calculate network diameter of each subgraph

In the code chunk below , the `graph_diameter()` function is used .
...
From the plot above, there are about 32 subgraphs with network diameter of 4 and above. We should investigate these subgraphs closer.
...
### 4.1.2 Betweenness scores across network diameter
...
### **4.1.3 Interactive netowrk graph of top 2 highest network diameter**
...
So far we have only explored the top 2 subgraphs with diameter. There are several other subgraphs with diameter of 4 that can be explored.

### 4.1.4 Find business owners who owns relatively more companies

If we would like to identify high diameter subgraphs that contains individual who own more than 1 company, we can simply do a group by `to` column of the `seafood_edgess_vis` dataframe and aggregate to find such people.

```{r}
big_players <- seafood_edges_vis %>% 
  group_by(to) %>% 
  mutate(no_of_companies_owned = n()) %>% 
  ungroup() %>% 
  select(from,to,type,no_of_companies_owned) %>% 
  arrange(desc(no_of_companies_owned))

```

Displaying the individuals who own more than one company in the datatable below. We can see two individual owners who owns three companies, and they are Adam Johnson and James Brown.

```{r}
d <- highlight_key(big_players) 


p<-ggplot(data=big_players, 
       aes(x=as.factor(no_of_companies_owned))) +
  geom_bar(fill = 'lightblue') +
  coord_flip() +
  theme_minimal() + 
...
Cross referencing with the seafood_nodes interative datatable in Section 4 above, if we search for "James Brown', we would see that he belongs to subgraph 291 and has a betweenness score of 119, which is rather high. Let us plot the network graph that is related to James brown.
...
visNetwork(sub227_nodes_vis, #%>% filter(betweenness_centrality >= 20),
           sub227_edges_vis,
           main = "Network graph of subgraph 291 with diameter = 4",
           height = "500px", width = "100%") %>%
  visIgraphLayout(layout = "layout_with_fr") %>%
  visEdges(smooth = list(enables = TRUE,
                         type= 'straightCross'),
...
The network diameter of this graph might not be the highest (of 6) however, it contains James Brown whom owns the most number of companies in the seafood dataset. There is also one other individual 'Adam Johnson' who is worth investigating.
...
Hovering the mouse above the points will reveal their subgraph membership numbers. The median of revenue per company is highest for subgraphs with higher diameter. Surprisely, there are also several revenue outliers in the smaller network diameter groups.

The top 10 subgraphs with the highest total revenue per company are listed below. The network diameters are mainly 1 or 2.

```{r}
#| echo: false
financial_risk_nodes %>%  slice(1:10) %>%  select(membership, revenue_per_company, network_diameter, no_of_companies_subgraph) %>%  kable()
```
...
The code chunk below is a continuation from section 4.2 previously. It creates a new column `no_of_countries_subgraph` where it computes the number of distinct values of countries present in each subgraph.
...
mutate(
    revenue_per_company = ifelse(no_of_companies_subgraph == 0, total_revenue_subgraph, round(total_revenue_subgraph / no_of_companies_subgraph))) %>% 
  arrange(desc(no_of_countries_subgraph)) %>% 
  select(membership, no_of_countries_subgraph, network_diameter,total_revenue_subgraph, no_of_companies_subgraph, revenue_per_company )
```
...
#| fig-width: 12
#| fig-asp: 0.618
d <- highlight_key(country_risk_nodes) 
...
In the above subgraph, the main company 'Aqua Aura SE Marine life' is registered in many countries like *Mawazam, Rio Isla, Icarnia, Oceanus, Nalakond, Coralmarica, Alverossia, Isliandor, Talandria*.

The second company ' BlueWater Bistro GmbH Industrial' company registered only in *Marebak*.

James Martinez is a beneficial owner to two companies in this subgraphs.
...
We have only explored the top 2 subgraphs with highest revenue. Many other subgraphs like 316, 173 and 193 could be investigated for their high revenue.

## 4.4 Finding similar subgraphs (groups)

In this section, we will use parallel coordinate plot to visualise and analyse multivariate, numerical data we have of each subgraphs. We hope to find patterns between different subgraphs. We will be comparing multiple variables
...
#| echo: false
#| eval: false
file_path1 <- "C:/Users/NYX/Desktop/TH_3/seafood_nodes_vis.csv"
file_path2 <- "C:/Users/NYX/Desktop/TH_3/seafood_edges_vis.csv"
file_path3 <- "C:/Users/NYX/Desktop/TH_3/edges_combined.csv"
write_csv(seafood_nodes_vis, file=file_path1)
write_csv(seafood_edges_vis, file=file_path2)
write_csv(edges_combined, file=file_path3)
```
