...
nodes_seafood_vis %>% 
  group_by(shpcountry) %>% 
  summarise(med = median(out_deg_centrality)) %>% 
  arrange(desc(med)) %>% 
  head(n=5)
```

```{r}
ggbetweenstats(data = nodes_seafood_vis %>% filter(shpcountry %in% c("Quirinelle", "Isla Solmar", "Osterivella", "Mawazam", "Kondanovia")), x = shpcountry, y = out_deg_centrality,
...
nodes_seafood_vis %>% 
  group_by(rcvcountry) %>% 
  summarise(med = median(in_deg_centrality)) %>% 
  arrange(desc(med)) %>% 
  head(n=5)
```

```{r}
ggbetweenstats(data = nodes_seafood_vis %>% filter(rcvcountry %in% c("Utoporiana", "Oceanus")), x = rcvcountry, y = in_deg_centrality,
               xlab = "Shipping Country", ylab = "In-Deg centrality",
               type = "np", pairwise.comparisons = TRUE, pairwise.display = "s",
               sort = "descending",
               sort.fun = median,
               mean.ci = T, p.adjust.method = "fdr",  conf.level = 0.95,
               title = "Comparison of Median In-Deg centrality across Shipping Countries") +
  scale_y_continuous(limits = c(0, 200)) +
   theme(axis.title.y=element_text(angle = 0,
                                  vjust=0.9))
```

Scatter plot of number of interaction and trading period between pairs of companies.
...
cor <- mc2_seafood_edges_agg_vis %>% 
  group_by(from, to) %>% 
  summarise(trading_days=as.integer(max(arrivaldate)-min(arrivaldate)+1),
            total_interaction = sum(Weight),
            median_interaction = median(Weight)) %>% 
  ungroup() %>% 
  arrange(trading_days)
```
...
#| fig-asp: 0.618
library(scales)
low_correl <- ggscatterstats(data = cor, 
                           x = trading_days, y = total_interaction,
                           type = "nonparametric") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_x_continuous(limits = c(0, 2600), 
                     breaks=seq(0, 2600, 365), 
                     labels= comma) + 
  scale_y_continuous(limits = c(0, 6500), 
                     breaks=seq(0, 6500, 1000), 
                     labels= comma) +
  
  labs(title = "Number of interactions and period of trading (days)", 
       x = "Days(365)", y = "Number of interactions") 
low_correl
```

We hope that from graphs like above, we can identify companies that has short period of interations but high number of interactions.
