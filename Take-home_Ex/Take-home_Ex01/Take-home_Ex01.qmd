...
The plot below shows us the pairs of months where wage are significantly different. See the next tab to see the error plots. Median Wage of March is signnicficantly different from all the wages of the other months.
...
Next, we will plot a coordinated dotplot to studying the distribution of expenses since we have the individual data of participant, which we can aggregated into annual expense.

First prepare the **annual** dataframe that contains participantId, aggregated monthly amounts and their demographics data.
...
### 3.1.5 Coordinated interactive dotplot of expenses by category

Hovering the cursor over the dots will show whether a participant have kids (True) or do not have kids.
