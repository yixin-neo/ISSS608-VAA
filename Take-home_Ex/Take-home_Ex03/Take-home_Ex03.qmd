...
financial_risk_nodes <- seafood_nodes_vis %>%
  ...
  summarize(
    ...
    no_of_companies_subgraph = as.numeric(sum(str_count(type_concat, "Company") * !grepl("Company Contacts", type_concat)))) %>% 
  ungroup() %>%
mutate(
...
The summary function tells us that there is an extreme outlier revenue_per_company value of \$55,376,193. We will exclude this outlier when we chart the dotplot.

```{r}
summary(financial_risk_nodes$revenue_per_company)
```

**Visualising the distribution of revenue per company in the remaining 650 subgraphs.**

```{r}
#| fig-width: 12
#| fig-asp: 0.618
p <- ggplot(data=financial_risk_nodes[-1, ],
            aes(x=revenue_per_company)) +
  geom_dotplot_interactive(aes(tooltip=as.factor(membership)),
                           stackgroups = TRUE,
                           binwidth = 10000,
                           method = "histodot") +
...
Hovering the mouse above the dotplot will reveal their subgraph numbers. The top 10 subgraphs with the highest total revenue per company (present in the subgraph) are listed below:
...
visNetwork(sub227_nodes_vis, #%>% filter(betweenness_centrality >= 20),
           sub227_edges_vis,
           main = "Network graph of subgraph 193 with diameter = 1",
           height = "500px", width = "100%") %>%
  visIgraphLayout(layout = "layout_nicely") %>%
  visEdges(smooth = list(enables = TRUE,
                         type= 'straightCross'),
...
In this subgraph, there are no companies involved. The revenue of 55376,193 comes from Callahan-Peterson and the product service he gives is 'Fishery'. Danielle Myers could be a business partner of Callahan-Peterson as the edge relationship is 'Beneficial owner'.
...
visNetwork(sub227_nodes_vis, #%>% filter(betweenness_centrality >= 20),
           sub227_edges_vis,
           main = "Network graph of subgraph 498 with diameter = 2",
           height = "500px", width = "100%") %>%
  visIgraphLayout(layout = "layout_with_fr") %>%
  visEdges(smooth = list(enables = TRUE,
                         type= 'straightCross'),
...
In this subgraph, the revenue of Sea Star LLC shipping is 1,507,513 and it has 10 beneficial owners with no other revenues reported. The product service offered by this company is 'Operation of fish hatcheries and preserves'. Interestingly, Matthew Williams is a company contacts of two companies offering unknown products services and also beneficial owners to two other companies; one of it is 'Wave Warriors S.A. de C.V. Express' company which has the highest betweenness score in the entire dataset. Its product services is "Beer, ale, and malt liquors.."
...
We will use a bar chart to visualise the count of countries
...
Subgraph 6 is very special so let us zoom in on its network graph.
