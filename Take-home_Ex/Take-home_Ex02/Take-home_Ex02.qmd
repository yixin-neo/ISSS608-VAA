...
kable(top_10_in_list)
```

Subset the dataset to filter records from top 10 receiving companies
...
Fresh Wharf SRL Consulting exhibits similar patterns to top shipping company Playa because of their close trading patterns.

## 3.3 Trading patterns of Volga River LLC Enterprises

Let us examine the trading patterns of Volga River LLC Enterprises, the second top ''in-deg' company with its four suppliers (as seen from the interactive graph).
...
Next, plot a time series using `geom_line()` and `geom_point()`.

```{r}
#| fig-width: 12
#| fig-height: 12
volga_in<- ggplot(volga_in_df, aes(x=arrivaldate, y=Weight)) +
  ...
  labs(title='Volga River LLC Enterprises activity with suppliers', 
         x = 'Date',
...
volga_in1<- ggplot(volga_in_df %>% filter(from=='Estrella de la Costa SRL'), aes(x=arrivaldate, y=Weight)) +
  ...
  geom_point_interactive(aes(tooltip= Weight), 
                           size = 0.5) +
  xlab("") +
  #facet_wrap(~from, ncol = 1)+
  theme_light() +
...
Analysis of the plot above: In the earlier years, Volga River LLC Enterprises receives goods from three shipping companies. However, it has been receiving only from Estrella de la Coasta SRL very frequently from 2032 Nov onwards. For our group project we can consider creating drop down list of 'receiving companies' for user to see each of their trading patterns with their suppliers.
...
The chart highlights actors with high out-deg centrality scores over 2028 to 2034.

## 3.3 Temporal analysis of top shipping company in trading occurrence over the years

Let us visualise the horizon plot of the number of trading activities of the top shipping company over the years.
...
Use the code chunk below to get the top 10 shipping companies
...
Observe the different shipping companies were active for different periods of time. Most of them have low interactions consistently throughout 2028-2034 (Like Estrella, Oceanic and Shou gan) while some have stop activity for some time (like Gujarat, Nautica and Diao er). The most unique of all is Playa del Tesoro OJSC - some of it interactions are extremely high for a specific period of time.

## 3.6 Treemap of business relationship between shipping and receiving companies

We somewhat know the high 'in-deg' and 'out-deg' scores of the filtered dataset. Lets us try to find business patterns by using a treemap.
